...
% Find number of frames in input file
%info = dicominfo(inputFile);
%nFrames = info.NumberOfFrames;

% Set coordinates
img1 = dicomread(inputFile, 'frames', 1);
%[positionBmode,threshBW, position,~] = segmentElastogram(img1);

     

% Get scale factor
%[scaleFactorHeight, scaleFactorWidth] = scale(img1, positionBmode, threshBW);
scaleFactor  = CalcScaleFactor( img1, positionBmode );
scaleFactorHeight = scaleFactor;
scaleFactorWidth = scaleFactor;
% scaleFactorHeight = CalcScaleFactor(img1, positionBmode(1:2));
% scaleFactorWidth = scaleFactorHeight;
% scaleFactor = CalcScaleFactor(img1, positionBmode(1,2));
% scaleFactorHeight = scaleFactor;
% scaleFactorWidth = scaleFactor;
...
for m = startFrame:endFrame
        ...
        while colBox < lenHor
            ...
            while rowBox < lenVer
                ...
                SWEImgBlock = replaceNonEValues(SWEImgBlock);%imgDiff);%
                
                % transform image into matrix containing elasticity values
                EMatrix = SWEtoKPa_muscles100(SWEImgBlock);
                
                % convert colour scale to max=300kPa instead of 180kPa
                %not necessary for 100 opacity as adjusted conversion
                %EMatrix300 = (300/180)*EMatrix;
                EMatrix300 = (inputMax / 300)*EMatrix; %prüfen
